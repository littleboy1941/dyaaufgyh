...
### ✅ Убрать рекламу и улучшенные превью ссылок (2026-09-19, собрано run 35441123062)

**Статус проверки на 2026-09-20:**

- **Реклама в каналах** — по первому впечатлению не показывается, проверено бегло глазами. Была ложная тревога: пользователь прислал скриншот с рекламой, но это оказался обычный Telegram, а не наша сборка.
- **Превью ссылок — работает, проверено на телефоне.** Главный риск фичи снят: сервер принимает адрес зеркала, которого нет в тексте сообщения, и сообщение отправляется нормально. Откатывать на «превью только у себя» не нужно.
- **Не проверено:** спонсорские результаты поиска, строка над списком чатов, «похожие каналы», секунды у времени, обратное выключение тумблеров.
...
- run 35432508169 — фикс краша экрана настроек (коммит c7974032). **11 минут**. **Актуальная сборка.**

- run 35441123062 — две правки статуса «в сети» (коммиты 9626f6fc, 301b71f3). **Актуальная сборка.** Перед ней отменён run 35440364404, чтобы не собирать половину фикса.

Итого 19 сентября: пять запусков, из них один упал (разорванный литерал), один отменён вручную, три успешных. Успешные после падения шли 11–20 минут — кэш переживает и падение, и отмену.

**Онлайн не гас после отправки сообщения (найдено на телефоне 2026-09-19).** Симптом: с включённым призраком отправляешь сообщение и остаёшься в сети, гаснешь только при выходе из приложения. Две независимые причины:

1. **Регрессия коммита `1706b46c`.** Он перенёс `ayuPushOfflineAfterSend` из `applySentMessage` в `beforeStarted` самого запроса — ради сокращения окна засветки на один round trip. Но офлайн стал уходить **до** того, как сервер пометит нас онлайн: опровергать было нечего, серверная отметка приходила следом и побеждала. Вызов возвращён после ответа сервера. **Держать оба варианта нельзя:** у пуша троттлинг в секунду, ранний вызов съедал бы правильный поздний. Пользователь вспомнил, что в утренней сборке призрака (run 35336542423) такого не было — так и есть, там вызов стоял после ответа.
2. **Не был закрыт весь остальной онлайн.** Сервер помечает аккаунт онлайн сам после значимых действий и сообщает об этом `updateUserStatus` про нас же. Отвечали мы на это ровно один раз, после отправки; всё остальное время, пока приложение открыто, опровергнуть было некому. Теперь этот апдейт сообщает событие в `AyuOnlineFlash.swift`, а `AccountPresenceManager` отвечает офлайном — при включённом призраке и не чаще раза в секунду (на наш офлайн сервер отвечает очередным апдейтом). В AyuGram Desktop то же решено воркером раз в три секунды; событийный вариант ближе к iOS.

**Важное про собственный статус:** клиент его нигде не хранит — своя строка в Postbox после логина прибита к `.present(until: Int32.max - 1)` локально (`Account.swift`), а серверный `updateUserStatus` про себя в базу не пишется (`AccountStateManagementUtils.swift`, ветка `peerId == accountPeerId` только считает `delayNotificatonsUntil`). Поэтому наблюдать свой реальный статус через Postbox бесполезно, единственный сигнал — этот апдейт.

**Урок:** порядок важнее скорости. Нельзя сократить окно засветки, отправив офлайн раньше причины онлайна. И: оптимизация задержки, сделанная «на глаз» без второго аккаунта под рукой, дважды за день ломала фичу.

**Краш при открытии экрана настроек (найден на телефоне 2026-09-19).** `ItemListControllerNode.swift:449` делает `assert(entries[i-1].isLessThan(entries[i]))`: список обязан идти строго по возрастанию `stableId`. Новые разделы стояли на экране перед призраком, но получили номера 900-1101 против 600-800 у призрака — порядок ломался на стыке, и в debug-сборке `assert` ронял приложение. Перенумеровано в промежуток 510-531, расположение разделов не изменилось.

**Вывод: у нас debug-сборка, в ней `assert` не игнорируется, а роняет приложение.** Любой нарушенный инвариант UI-списка — это краш у пользователя, а не тихая кривизна. Компилятор такие вещи не видит.

**Проверка перед сборкой: `ayu-docs/precheck.py`.** Собирает порядок записей настроек ровно так же, как `ayuSettingsEntries`, и сверяет `stableId`; ищет незакрытые строковые литералы и несходящиеся скобки в наших файлах (с разбором строк и комментариев, не счётом символов). Проверено, что на коммите 805cbb32 он находит этот краш. Гонять перед каждым запуском сборки.
