...
Состояние на 2026-09-17. Это форк официального Telegram-iOS с фичами AyuGram, который собирается без Mac на GitHub Actions и ставится на iPhone через SideStore с бесплатным Apple ID.
...
- **TODO:** `timeout-minutes` на шаге Build короче job-таймаута, чтобы при зависании сохранялся кэш; расширенный монитор (`memory_pressure`, `df`, top по RSS) в файл и выгрузка при падении.
...
### ▶ Следующая задача (с 2026-09-18): анимация удаления

Когда собеседник удаляет сообщение при открытом чате, проигрывать анимацию. Только в момент удаления; при повторном открытии чата просто показывается 🗑.

Варианты (выбрать с пользователем, предпочтение — 1):
1. Эффект «пыли» Telegram (DustEffect) на пузыре, затем пузырь снова проявляется полупрозрачным и с 🗑.
2. Мигание или вздрагивание, переход в полупрозрачность, 🗑 появляется с анимацией.
3. Только анимированное появление 🗑.

Технический план:
- Ловить момент в UI: у сообщения появился `AyuDeletedMessageAttribute` при обновлении истории. `TelegramCore` не трогать, чтобы сборка была быстрее.
- Сначала Codex исследует, как устроен и где вызывается DustEffect и как применить его к сообщению, которое остаётся.
- Заодно проверить теорию, что сборка без изменения публичного API `TelegramCore` заметно быстрее часа.
