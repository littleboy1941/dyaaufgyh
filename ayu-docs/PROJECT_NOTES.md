...
### 🔴 СЛОМАНО: статус «в сети» висит всегда (найдено 2026-09-19, чинить в первую очередь)
...
**Предлагаемый фикс:** `wasOnline` должен отслеживать сырое `shouldKeepOnline`, а не вычисленное значение, плюс отдельно запоминать предыдущее `hidesOnline` и вызывать `updatePresence(shouldKeepOnline)` при изменении любого из двух. Предохранитель внутри `updatePresence` сам превратит вызов в offline. Тогда:

- свернул приложение с призраком → offline уходит;
- развернул → тоже уходит offline (безвредно и полезно);
- включил призрак на ходу → offline уходит сразу (ради чего коммит и делался).

**Обходной путь до фикса:** выключить подпункт «Не показывать себя в сети», главный тумблер можно оставить.
...
0. **СРОЧНО:** починить статус «в сети» — см. раздел «🔴 СЛОМАНО» выше. Регрессия коммита `0d32a428`, диагноз и предлагаемый фикс записаны, остаётся применить и собрать.
